...
public final class QueryParse {
    ...
    public static int parse(String stmt) {
        for (int i = 0; i < stmt.length(); ++i) {
            switch (stmt.charAt(i)) {
                ...
                case 'N':
                case 'n':
                    return nativeCheck(stmt, i);
                case 'J':
                case 'j':
                    return jobCheck(stmt, i);
                case 'F':
                case 'f':
                    return flushCheck(stmt, i);
                default:
                    return OTHER;
            }
        }
    ...
    static int createSchemaAndCommitAndCache(String stmt, int offset) {
        ...
        if (StrUtil.startWithIgnoreCase(stmt, "cache")) {
            return CACHE;
        }
    ...
    // JOB' '
    static int jobCheck(String stmt, int offset) {
        if (stmt.length() > offset + 3) {
            char c1 = stmt.charAt(++offset);
            char c2 = stmt.charAt(++offset);
            char c3 = stmt.charAt(++offset);
            if ((c1 == 'o' || c1 == 'O') && (c2 == 'b' || c2 == 'B')
                    && (c3 == ' ' || c3 == '\t' || c3 == '\r' || c3 == '\n')) {
                return (offset << 8) | JOB;
            }
        }
    ...
    // FLUSH' '
    static int flushCheck(String stmt, int offset) {
        if (stmt.length() > offset + 5) {
            char c1 = stmt.charAt(++offset);
            char c2 = stmt.charAt(++offset);
            char c3 = stmt.charAt(++offset);
    ...
    // NATIVE' '
    static int nativeCheck(String stmt, int offset) {
        if (stmt.length() > offset + 6) {
            char c1 = stmt.charAt(++offset);
            char c2 = stmt.charAt(++offset);
            char c3 = stmt.charAt(++offset);
