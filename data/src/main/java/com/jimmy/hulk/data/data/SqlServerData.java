...
public class SqlServerData extends TransactionData {
    ...
    public int addBatch(List<Map<String, Object>> docs) {
        ...
        for (Map<String, Object> doc : docs) {
            ...
            executeBody.setDoc(doc);
            executeBody.setTableName(indexName);
    ...
    public List<Map<String, Object>> queryPageList(Wrapper wrapper, Page page) {
        if (CollUtil.isEmpty(wrapper.getQueryPlus().getOrders())) {
            wrapper.order("1");
        }
        ...
        String querySQL = new StringBuilder(sql).append(" offset ").append(page.getPageNo() * page.getPageSize()).append(" rows fetch next ").append(page.getPageSize()).append(" rows only").toString();
    ...
    public List<Map<String, Object>> queryRange(Wrapper wrapper, Integer start, Integer end) {
        ...
        ConditionPart conditionPart = this.conditionTrans(wrapper.getQueryPlus());
        String sql = StrUtil.format(this.selectColumnHandler(wrapper, QUERY_LIST_TEMPLATE), indexName, conditionPart.getConditionExp());
        String querySQL = new StringBuilder(sql).append(" offset ").append(start).append(" rows fetch next ").append(end).append(" rows only").toString();
    ...
    public Map<String, Object> queryOne(Wrapper wrapper) {
        ConditionPart conditionPart = this.conditionTrans(wrapper.getQueryPlus());
        String sql = StrUtil.format(this.selectColumnHandler(wrapper, QUERY_LIST_TEMPLATE), indexName, conditionPart.getConditionExp()) + " offset 0 rows fetch next 1 rows only";
