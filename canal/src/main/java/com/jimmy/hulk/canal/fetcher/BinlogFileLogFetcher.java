package com.jimmy.hulk.canal.fetcher;
...
public final class BinlogFileLogFetcher extends LogFetcher {
    ...
    public void open(String urlStr, final long filePosition) throws FileNotFoundException, IOException {
        ...
        if (buffer[0] != BINLOG_MAGIC[0] || buffer[1] != BINLOG_MAGIC[1] || buffer[2] != BINLOG_MAGIC[2] || buffer[3] != BINLOG_MAGIC[3]) {
        ...
        if (filePosition > BIN_LOG_HEADER_SIZE) {
            final int maxFormatDescriptionEventLen = FormatDescriptionLogEvent.LOG_EVENT_MINIMAL_HEADER_LEN + FormatDescriptionLogEvent.ST_COMMON_HEADER_LEN_OFFSET + LogEvent.ENUM_END_EVENT + LogEvent.BINLOG_CHECKSUM_ALG_DESC_LEN + LogEvent.CHECKSUM_CRC32_SIGNATURE_LEN;
